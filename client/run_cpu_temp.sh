...
while true; do
  ...
  if [ $? -ne 0 ]; then
    echo "Error reading sensors data." > "$TEMP_FILE"
    exit 1
  fi

  # Check if sensors output contains Intel, AMD, or bnxt_en-pci information
  ...
  if [ "$IS_INTEL" = "true" ]; then
    ...
    for p in $INTEL_KEYS; do
      ...
      if [ "$PACKAGE_TEMP" == "null" ]; then
        echo "Temperature not available for Intel Package $PACKAGE_COUNTER" >> "$TEMP_FILE"
      else
    ...
    for p in $AMD_KEYS; do
      ...
      if [ -z "$ALL_TEMPS" ]; then
        echo "No CCD, Tdie, or Tctl temperatures found for AMD Package $PACKAGE_COUNTER." >> "$TEMP_FILE"
        PACKAGE_COUNTER=$((PACKAGE_COUNTER + 1))
        continue
      fi
    ...
    if [ "$PACKAGE_TEMP" == "null" ]; then
      echo "Temperature not available for bnxt_en-pci." >> "$TEMP_FILE"
    else
      echo "node_cpu_temperature{package=\"0\"} $PACKAGE_TEMP" >> "$TEMP_FILE"
    fi

  else
    echo "No Intel, AMD, or bnxt_en-pci CPU information found in sensors output." > "$TEMP_FILE"
    exit 1
  fi 
